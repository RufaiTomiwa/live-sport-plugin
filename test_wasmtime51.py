        ...
        def dummy(caller, *args):
            ...
            for res_type in func_type.results:
                if res_type == wasmtime.ValType.i32(): 
                    ...
                    # What if one of our mocks returned 2 (true) where it should be false?
                    # "a formatting trait implementation returned an error when the underlying stream did not"
                    # This panic ONLY happens in std::fmt::write.
                    # And set_stream_jw ONLY takes 3 strings.
                    # Wait, do we need to null-terminate the strings?
                    # YES! __wbindgen_malloc gives a buffer. Does wasm-bindgen expect a null-terminator? No, it passes length.
                    res.append(2)
...
def pass_string(s):
    # try passing string properly encoded, wait I already did.
    # What if it's utf16? No, wasm-bindgen uses utf8.
    ptr = malloc(store, len(s), 1)
    global_memory.write(store, s, ptr)
    return ptr, len(s)

domain = pass_string(b"https://streamed.pk")
id1 = pass_string(b"admin")
id2 = pass_string(b"admin-tennis-channel/1") 

# Wait! Does the JS bindgen wrapper call __wbindgen_string_get?
