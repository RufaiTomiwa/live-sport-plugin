        ...
        def dummy(caller, *args):
            ...
            for res_type in func_type.results:
                if res_type == wasmtime.ValType.i32(): 
                    ...
                    # If this is returning a new JS object, we just return an ID
                    obj = {"id": name, "c": wbg_counter}
                    wbg_objs[wbg_counter] = obj
                    res.append(wbg_counter)
                    wbg_counter += 1
            
            if name == "__wbg_new_b5d9e2fb389fef91":
                ptr, length = args[0], args[1]
                mem = global_memory.read(store, 0, global_memory.data_len(store))
                s = mem[ptr:ptr+length]
                # It's actually passing an error message or some string to a JS constructor like 
ew Error(s)
                calls.append(f"__wbg_new_b5d9e2fb389fef91 (maybe new Error?) called with RAW length {length}")
            elif name == "__wbindgen_string_get":
                # Wait, string get returns 2 i32s via pointers!
                # Actually __wbindgen_string_get signature: (i32, i32) -> void
                # Wait, looking at the wasm imports, what is it?
                calls.append(f"{name} called with args: {args}")
...
try:
    instance.exports(store)["set_stream"](store, domain[0], domain[1], id1[0], id1[1], id2[0], id2[1], None)
