...
for i in module.imports:
    def make_dummy(name, func_type):
        def dummy(caller, *args):
            # Try to print string parameters
            str_args = []
            for arg in args:
                # If arg is around 1 million, it's likely a pointer
                if isinstance(arg, int) and 1000000 < arg < 2000000:
                    try:
                        # Just grab 32 bytes and see if it looks like a string
                        mem = global_memory.read(store, arg, 64)
                        if b"window" in mem or b"document" in mem or b"fetch" in mem or b"Error" in mem:
                            str_args.append(mem)
                    except:
                        pass
            ...
            for res_type in func_type.results:
                if res_type == wasmtime.ValType.i32() or res_type == wasmtime.ValType.i64():
                    if "fetch" in name:
                        res.append(2) # Return a valid externref idx
                    elif "window" in name:
                        res.append(3)
