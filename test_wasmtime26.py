...
# The values passed were 1048572 and 1080152.
print("Values are", 1048572, "and", 1080152)
# Could the string be around there?
for offset in range(1048572 - 100, 1048572 + 100):
    try:
        mem = global_memory.read(store, offset, 50)
        if b"Error" in mem or b"window" in mem or b"Cannot" in mem:
            print("Found at", offset, mem)
    except: pass
for offset in range(1080152 - 100, 1080152 + 100):
    try:
        mem = global_memory.read(store, offset, 50)
        if b"Error" in mem or b"window" in mem or b"Cannot" in mem:
            print("Found at", offset, mem)
