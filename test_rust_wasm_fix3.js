...
async function run() {
  ...
  // Replace xport function foo with window.foo = function foo
  js = js.replace(/export\s+function\s+([A-Za-z0-9_]+)/g, 'window.$1 = function $1');
  
  // Remove xport default init;
  ...
  if (dom.window.init_wasm) {
      dom.window.init_wasm();
  } else {
      console.log('No init_wasm available?');
  }
