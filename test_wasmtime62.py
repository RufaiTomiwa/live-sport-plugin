...
for i in module.imports:
    def make_dummy(name, func_type):
        def dummy(caller, *args):
            wbg_calls[name] = wbg_calls.get(name, 0) + 1
            if "wbg" in name:
...
def read_str(ptr, length):
    return mem[ptr:ptr+length].decode('utf-8', errors='replace')
...
for c in calls:
    print(c[0])
    if c[0] == '__wbindgen_string_get':
        print("  Arg:", c[1])
    if c[0] == '__wbg_new_b5d9e2fb389fef91':
        print("  Args:", c[1])
        try:
            print("  String:", read_str(c[1][0], c[1][1]))
        except:
            pass
