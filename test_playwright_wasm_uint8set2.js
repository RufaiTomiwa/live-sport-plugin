...
(async () => {
  ...
  await page.addInitScript(() => {
    ...
    Uint8Array.prototype.set = function(array, offset) {
        if (window.fetchDone && array && array.length === window.fetchBodyLen) {
            ...
            // Now dump the WASM memory around this pointer immediately!
            if (window.wasmInstance) {
                const mem = new Uint8Array(window.wasmInstance.exports.memory.buffer);
                if (offset > 1024 && offset + array.length + 1024 < mem.length) {
                    const dump = mem.subarray(offset - 1024, offset + array.length + 1024);
                    window.memAroundCiphertext = Array.from(dump);
                }
            }
        }
  ...
  const dump = await page.evaluate(() => window.memAroundCiphertext);
  if (dump) {
      fs.writeFileSync('memAroundCiphertext.bin', Buffer.from(dump));
      console.log('Saved memAroundCiphertext.bin');
  }
