...
async function run() {
  ...
  virtualConsole.on("jsdomError", (e) => { /* ignore */ });
  virtualConsole.on("log", (msg) => { console.log('Log:', msg); });
  
  const dom = new JSDOM('<html lang="en"><body><div id="player"></div><script>window.performance = { timing: { navigationStart: Date.now() }, now: () => 1000 };</script><script src="https://strmd.b-cdn.net/js/bundle-jw.js"></script></body></html>', {
