...
def run():
    ...
    module = wasmtime.Module.from_file(engine, "lock2.wasm")
    ...
    try:
        ...
        print("Caught exception in set_stream:", e)

    mem = global_memory.read(store, 0, global_memory.data_len(store))
    with open("wasm_mem_crash.bin", "wb") as f:
        f.write(mem)
    print("Dumped memory to wasm_mem_crash.bin (Size: {} bytes)".format(len(mem)))
