        ...
        def dummy(caller, *args):
            ...
            for res_type in func_type.results:
                if res_type == wasmtime.ValType.i32(): 
                    # If it's asking for string length, maybe we return 0? No, this is for any bool.
                    # wait, __wbindgen_is_string is imported?
                    if name == "__wbindgen_is_string": res.append(1)
                    else: res.append(2)
...
def pass_string(s):
    # wasm-bindgen malloc actually just takes (len, alignment)
