...
async function run() {
    ...
    // Fake DOM
    const document = {
        location: { pathname: '/embed/admin/admin-tennis-channel/1', href: 'https://embed.st/embed/admin/admin-tennis-channel/1' }
    };
    const window = { document, navigator: { userAgent: "Node" } };
    
    const heap = [null, undefined, null, true, false, window, document, window.navigator];
    ...
    importNames.forEach(name => {
        imports['./locked_bg.js'][name] = function(...args) {
            ...
            return 0; // Return dummy pointer or object index
        }
    });
    
    imports['./locked_bg.js']['__wbg___wbindgen_string_get_72fb696202c56729'] = function(arg0, arg1) {
        const obj = getObject(arg1);
        console.log('WASM string get:', arg1, obj);
        if (typeof obj === 'string') {
            const buf = Buffer.from(obj, 'utf8');
            const ptr = instance.exports.__wbindgen_malloc(buf.length);
            Buffer.from(memObj.buffer).set(buf, ptr);
            const view = new Int32Array(memObj.buffer);
            view[arg0 / 4 + 1] = buf.length;
            view[arg0 / 4 + 0] = ptr;
        }
    }
    
    imports['./locked_bg.js']['__wbg_static_accessor_WINDOW_f8727f0cf888e0bd'] = function() {
        return addHeapObject(window);
    }
    imports['./locked_bg.js']['__wbg_document_ee35a3d3ae34ef6c'] = function() {
        return addHeapObject(document);
    }
    imports['./locked_bg.js']['__wbg_fetch_e6e8e0a221783759'] = function() {
        return addHeapObject(Promise.resolve({}));
    }
    ...
    try {
        ...
        instance.exports.init_wasm();
        
        const urlStr = 'https://embed.st/embed/admin/admin-tennis-channel/1';
        const urlBuf = Buffer.from(urlStr, 'utf8');
        const urlPtr = instance.exports.__wbindgen_malloc(urlBuf.length);
        Buffer.from(memObj.buffer).set(urlBuf, urlPtr);
        
        console.log('Calling set_stream...');
        try {
            instance.exports.set_stream(urlPtr, urlBuf.length);
        } catch (e) {
